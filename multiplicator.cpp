...
namespace weighted
{
  ...
  csr_matrix* mult(csc_matrix* A, Biclique* b)
  {
    ...
    while (not Hr.empty()) {
      ...
      Hr.pop();
      Hc.push(elem);
      msg(6)  << "pop Hr " << " | " << elem->start_col << " " << elem->end_col << " | " << elem->start_row << " " << elem->end_row << " | " << elem->value_col << " " << elem->value_row << msgEndl;
      ...
      if (Hr.empty() or (Hr.top()->value_col != elem->value_col)) { //primer elemento de la columna es distinto
        ...
        while (not Hc.empty()) {
          ...
          msg(6) << "pop Hc " << "inter: " << inter->start_col << " " << inter->end_col << " | " << inter->start_row << " " << inter->end_row << " | " << inter->value_col << " " << inter->value_row << msgEndl;
          ...
          if (Hc.empty() or inter->value_row != Hc.top()->value_row) { // si queda vacio o si el siguiente valor es distinto, push en csr
            ...
            msg(6) <<"(" << A->row_ind[inter->start_col] << ", " << A->row_ind[inter->start_row] << ") = " << sum << msgEndl;
            sum = 0;
          } 
          ...
          if (inter->start_row < inter->end_row - 1) {
            ...
            msg(6) << "inter modified:"  << inter->start_col << " " << inter->end_col << " | " << inter->start_row << " " << inter->end_row << " | " << inter->value_col << " " << inter->value_row << msgEndl;
    ...
    msg(6) << "edges computed: " << csr_res->values.size() << msgEndl;
    return csr_res;   
  ...
  csr_matrix* mult(Biclique* a, Biclique* b) //bxb
  {
    ...
    for (size_t i = 0; i < a_csc->size(); ++i) {
      ...
      if (count > 1) {
        std::sort(C_temp.begin(), C_temp.end());
        for (auto& k : C_temp) {
          msg(3) << k.first << "," << k.second << msgEndl;
          if (p.C.empty() or p.C.back().first != k.first) {
            p.C.push_back(k);
          } else {
            p.C.back().second += k.second;
